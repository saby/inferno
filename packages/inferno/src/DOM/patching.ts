import { combineFrom, isFunction, isInvalid, isNull, isNullOrUndef, isArray } from 'inferno-shared';
...
function replaceWithNewNode(lastVNode, nextVNode, parentDOM: Element, context: Object, isSVG: boolean, lifecycle: Function[], isRootStart?: boolean) {
  ...
  if ((nextVNode.flags & lastVNode.flags & VNodeFlags.DOMRef) !== 0) {
    ...
    if (isRootStart && lastVNode.dom === parentDOM) {
      ...
      const lastChildren: VNode[] = lastVNode.children;
...
function patchPortal(lastVNode: VNode, nextVNode: VNode, context, lifecycle: Function[]) {
  ...
  const fixedNextChildren = fixPortalChildrenForWasaby(nextChildren);
  if (lastContainer !== nextContainer && !isInvalid(fixedNextChildren)) {
    const node = fixedNextChildren.dom as Element;
