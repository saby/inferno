...
function DEV_ValidateKeys(vNodeTree, forceKeyed) {
    ...
    for (var i = 0, len = vNodeTree.length; i < len; ++i) {
        ...
        if (foundKeys[key]) {
            ...
            key = duplicateKeys(String(childNode.key), foundKeys);
            childNode.key = key;
...
function fixPortalKeyForWasaby(children) {
    if (isInvalid(children)) {
        return null;
    }
    if (isArray(children)) {
...
function createPortal(children, container) {
    return createVNode(1024 /* Portal */, 1024 /* Portal */, null, children, 0 /* UnknownChildren */, null, fixPortalKeyForWasaby(children), container);
}
...
// Wasaby Synchronizer can work only with array children.
// Inferno and Wasaby Synchronizer will be removed soon, so let's choose simplier way.
function fixPortalChildrenForWasaby(children) {
    if (isArray(children)) {
        return children[0];
    }
    return children;
...
function parseHTML(markup) {
    var result = getDOMParser().parseFromString(markup, 'text/html');
    return result.body.firstChild;
}
function mountHTML(vNode, parentDom, nextNode) {
    // @ts-ignore
    var dom = (vNode.dom = parseHTML(vNode.markup));
    if (!isNull(parentDom)) {
...
// Wasaby Synchronizer can work only with array children.
// Inferno and Wasaby Synchronizer will be removed soon, so let's choose simplier way.
function fixPortalChildrenForWasaby$1(vNode) {
...
function mountPortal(vNode, context, parentDOM, nextNode, lifecycle) {
    mount(fixPortalChildrenForWasaby$1(vNode), vNode.ref, context, false, null, lifecycle);
...
// Wasaby Synchronizer can work only with array children.
// Inferno and Wasaby Synchronizer will be removed soon, so let's choose simplier way.
function fixPortalChildrenForWasaby$2(children) {
    if (isArray(children)) {
        return children[0];
    }
    return children;
...
function patchPortal(lastVNode, nextVNode, context, lifecycle) {
    ...
    nextVNode.dom = lastVNode.dom;
    var fixedNextChildren = fixPortalChildrenForWasaby$2(nextChildren);
    if (lastContainer !== nextContainer && !isInvalid(fixedNextChildren)) {
        var node = fixedNextChildren.dom;
